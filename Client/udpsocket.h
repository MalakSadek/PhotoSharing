...
public:
    ...
    int readFromSocketWithTimeout(int sock, char message1[BYTESIZE], char message2[BYTESIZE], size_t message2length, size_t length, int x, sockaddr_in aSocketAddress, int aLength, int timeout) {
        int retransmit = 0;
        struct timeval tv;
        //if (timeout < 100) {
        tv.tv_sec = timeout;
    ...
    int index = 0;
    sockaddr_in readFromSocketWithBlock(int sock, char message1[BYTESIZE], size_t length, int x, sockaddr_in aSocketAddress, int aLength) {
        ssize_t n;
        if((n = recvfrom(sock, message1, length, 0, (struct sockaddr*) &aSocketAddress, (socklen_t*)&aLength))<0) {
